...
Usage:
  python experiments/compute_category_avg.py --type event
  python experiments/compute_category_avg.py --type person
"""
    ...
    for category in os.listdir(config_dir):
        ...
        for fname in os.listdir(cat_path):
            if not fname.endswith(".json"):
                continue
            # e.g. french_revolution_config_wikidata.json -> french_revolution
...
def main(entity_type: str):
    ...
    for entity, category in sorted(category_map.items()):
        ...
        if event_driven is None and baseline is None:
            print(f"[SKIP] {entity}: no score files found")
            continue
        if event_driven:
